...
class DRResidualDataset(Dataset):
    def __init__(self, orig_npz_path: str, dr_pred_npz_path: str, dr_res_npz_path: str, norms: dict | None = None):
        ...
        self.X = d_orig["X"].astype(np.float64, copy=False)           # (N, T, F)
        self.y_mask = d_res["y_mask"].astype(np.int8, copy=False)      # (N, H)
        self.Y_pred = d_pred["Y_pred"].astype(np.float64, copy=False)  # (N, H, D)
        self.Y_res = d_res["Y_res"].astype(np.float64, copy=False)     # (N, H, D)
        self.pred_len = d_orig.get("pred_len", self.y_mask.sum(axis=1)).astype(np.int64)  # (N,)
...
def build_dataloaders(orig_dir: str, dr_out_dir: str, batch_size: int = 64, num_workers: int = 0,
    ...
    for split in ["train", "val", "test"]:
        o, p, r = paths(split)
        ds = DRResidualDataset(o, p, r, norms=norms)
