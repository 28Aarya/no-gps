...
class AircraftTrajectoryTransformer(nn.Module):
    """
    Sequence-to-Point Transformer for Aircraft Trajectory Prediction
    Encoder: Processes full trajectory sequence (60 time steps)
    Decoder: Outputs single prediction point (lat, lon coordinates)
    """
    def __init__(self, 
        ...
        # Decoder layers (for sequence-to-point)
        self.decoder_layers = nn.ModuleList([
            DecoderLayer(d_model, num_heads, d_ff, dropout) 
            for _ in range(num_layers)
        ])
        
        # Decoder input projection (single token for prediction)
        self.decoder_input_projection = nn.Linear(2, d_model)  # 2 for lat/lon
        
        # Output projection for prediction (lat, lon coordinates)
    ...
    def generate_mask(self, src, tgt):
        ...
        # Target mask (decoder) - causal mask for single token
        tgt_mask = torch.ones(tgt.size(0), 1, 1, 1, dtype=torch.bool, device=tgt.device)
    ...
    def forward(self, 
        ...
        """
        Forward pass for sequence-to-point trajectory prediction
        ...
        # Create decoder input (single token for prediction)
        # Use zeros as initial decoder input - the decoder will learn to predict from encoder context
        tgt = torch.zeros(batch_size, 1, 2, device=input_ids.device)  # (batch_size, 1, 2)
        ...
        # Decoder: Process single token to get prediction
        dec_output = tgt_embedded
        for dec_layer in self.decoder_layers:
            dec_output = dec_layer(dec_output, enc_output, src_mask, tgt_mask)
        
        # Output projection for prediction (lat, lon coordinates)
        predictions = self.output_projection(dec_output)  # (batch_size, 1, 2)
